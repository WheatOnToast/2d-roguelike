...
void camera_world_snap()
{
	...
	if (camera.position.x + res.x > camera.size.x) {
		camera.position.x = camera.size.x - res.x;
	}
	if (camera.position.y + res.y > camera.size.y) {
		camera.position.y = camera.size.y - res.y;
	}
}
